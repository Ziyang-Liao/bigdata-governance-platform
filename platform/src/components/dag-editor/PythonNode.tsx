...
import { CodeOutlined, SyncOutlined, CheckCircleOutlined, CloseCircleOutlined, ClockCircleOutlined } from "@ant-design/icons";
...
function PythonNode({ data }: NodeProps) {
  const st = statusStyle[data.runStatus] || { border: "#faad14", bg: "#fffbe6", icon: <CodeOutlined style={{ color: "#faad14" }} /> };
  return (
    <div style={{ padding: "8px 16px", border: `2px solid ${st.border}`, borderRadius: 8, background: st.bg, minWidth: 120 }}>
      <Handle type="target" position={Position.Top} />
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        {data.runStatus ? st.icon : <CodeOutlined style={{ color: "#faad14" }} />}
